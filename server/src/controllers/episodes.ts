...
const getEpisodesBySeason = async (req: Request, res: Response, next: NextFunction) => {
...
const deleteEpisode = async (req: Request, res: Response, next: NextFunction) => {
  const id: number = +req.params.id;
...
const addEpisode = async (req: Request, res: Response, next: NextFunction) => {
  const episode = await prismaClient.episode.create({
    data: extractEpisodeData(req, +req.body.seasonId)
  });
...
export default { getEpisodes, getEpisodesBySeason, getEpisode, updateEpisode, deleteEpisode, addEpisode };
